...
Статус: исполнительная проектная документация CORE-2, обновлена после AUDIT PASS по решениям заказчика; `TASK-PC2-002`, `TASK-PC2-003` and `TASK-PC2-004` closed / ready for commit after test and audit PASS. Full CORE-2 remains in progress and is not release-complete.
...
| `TASK-PC2-004 Operation Row FK Enrichment` | CLOSED / READY FOR COMMIT | Deterministic nullable `OperationDetailRow.marketplace_listing` FK enrichment only. Design handoff in `docs/reports/TASK_PC2_004_DESIGN_HANDOFF.md`; test PASS in `docs/testing/TEST_REPORT_TASK_PC2_004_OPERATION_ROW_FK_ENRICHMENT.md`; audit PASS in `docs/audit/AUDIT_REPORT_TASK_PC2_004_OPERATION_ROW_FK_ENRICHMENT.md`; closeout report in `docs/reports/TASK_PC2_004_OPERATION_ROW_FK_ENRICHMENT_REPORT.md`. `product_ref` checksum evidence accepted; summary/action rows remain unlinked. |

Only `TASK-PC2-002`, the narrowed `TASK-PC2-003` API slice and `TASK-PC2-004` are marked closed here. Full CORE-2 remains in progress and is not marked release-complete.
