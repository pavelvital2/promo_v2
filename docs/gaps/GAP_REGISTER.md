...
На 2026-04-26 новых открытых GAP для проектирования Stage 2.1 не создано. Потенциальные кандидаты из `docs/source/stage-inputs/tz_stage_2.1.txt` закрыты самим ТЗ как рекомендуемые проектные решения и зафиксированы в ADR-0017..ADR-0020:
...
- Blocking gate: нет, закрыт решением заказчика 2026-04-30
- Где обнаружен: `docs/source/stage-inputs/preliminary_stage_2_2_ozon_api.txt`, TASK-018 discovery
...
## Stage 3 / CORE-2 Product Core Integration GAP evaluation

На 2026-05-02 CORE-2 documentation package passed audit and recheck. После AUDIT PASS заказчик дал решения по `GAP-CORE2-001`..`GAP-CORE2-005`; customer-decision blockers сняты. После rework по TASK-PC2-003 заказчик также закрыл `GAP-CORE2-006`; `GAP-CORE2-007` отложен в future task и не блокирует суженный API linkage slice TASK-PC2-003. Entries remain here as resolved/deferred decision records with implementation constraints. Product code, migrations and tests are still not implemented by these documentation updates.

### GAP-CORE2-001: ProductVariant auto-create mode

- Статус: resolved/customer_decision
- Phase gate: closed_for_core_2_auto_create_imported_draft_implementation
- Blocking gate: нет, closed by customer decision 2026-05-02. Implementation remains constrained by fixed SKU validator, imported/draft lifecycle, audit/history and tests.
- Где обнаружен: `docs/tasks/design/product-core/TZ_CORE_2_PRODUCT_CORE_INTEGRATION_FOR_CODEX_DESIGNER.md` §7.3, §10
- Требование ТЗ/task: decide whether CORE-2 creates draft/imported `ProductVariant` by unified seller article
- Затронутая область: normalized article linkage, ProductVariant lifecycle, InternalProduct shell creation, UI review, audit/history, exports
- Почему нельзя продолжать без решения: auto-creating variants changes product identity workflow and may create user-visible draft records; designer must not choose this business behavior by assumption
- Варианты решения:
  - A: do not auto-create; leave listing unmatched and require manual create/map
  - B: create imported/draft ProductVariant under strict conditions with review/audit/report
  - C: create active ProductVariant and confirmed mapping automatically
- Рекомендация проектировщика: B, but only with explicit imported/draft lifecycle and customer-approved rule for required parent `InternalProduct`
- Требуется эскалация заказчику через оркестратора: закрыта 2026-05-02
- Решение: Option B approved. If API listing article already matches valid internal SKU format and an existing active `ProductVariant` is found, link listing to it, set mapping `matched`, and write audit/history. If no variant exists and no conflict exists, auto-create `InternalProduct` + `ProductVariant` as imported/draft, link listing, set mapping `matched`, and write audit/history. No fuzzy matching. Invalid/non-unified external articles create/update `MarketplaceListing` only and do not auto-create variants.
- Дополнительное ограничение: `matched` records the listing-to-variant link; imported/draft variant review state must remain visible until manual review/confirmation/archive. Product code is not implemented by this decision.
- Дата решения: 2026-05-02
- ADR: ADR-0045

### GAP-CORE2-002: Approved WB/Ozon listing sources

- Статус: resolved/customer_decision_with_endpoint_artifact_gate
- Phase gate: closed_for_core_2_read_only_listing_source_policy
- Blocking gate: нет for policy; concrete endpoint use remains gated by official documentation evidence, pagination/rate/retry/redaction contract and mocks.
- Где обнаружен: `docs/tasks/design/product-core/TZ_CORE_2_PRODUCT_CORE_INTEGRATION_FOR_CODEX_DESIGNER.md` §7.1, §10
- Требование ТЗ/task: identify approved API endpoints for complete WB/Ozon listings in CORE-2 and do not invent endpoints
- Затронутая область: WB/Ozon listing sync, full catalog refresh, source fields, rate limits, permissions, mocks
- Что уже есть в документации: WB `GET /api/v2/list/goods/filter`; WB regular promotion endpoints; Ozon Stage 2.2 Elastic-scoped sources `/v1/actions`, `/v1/actions/products`, `/v1/actions/candidates`, `/v3/product/info/list`, `/v4/product/info/stocks`
- Решение: CORE-2 may add official read-only WB/Ozon catalog/listing APIs. Every concrete endpoint added by implementation must have official docs confirmation, pagination/rate limits/retries/redaction rules and mock/contract tests. CORE-2 must not add marketplace write endpoints.
- Remaining implementation gate: if an implementation task cannot provide endpoint-specific official docs evidence and tests, that endpoint is blocked for that task. This is an artifact/implementation gate, not an open customer decision.
- Дата решения: 2026-05-02
- Требуется эскалация заказчику через оркестратора: закрыта for read-only listing source policy
- ADR: ADR-0042, ADR-0046

### GAP-CORE2-003: OperationDetailRow enrichment scope

- Статус: resolved/customer_decision
- Phase gate: closed_for_core_2_operation_fk_backfill_and_new_row_enrichment_scope
- Blocking gate: нет, closed by customer decision 2026-05-02
- Где обнаружен: `docs/tasks/design/product-core/TZ_CORE_2_PRODUCT_CORE_INTEGRATION_FOR_CODEX_DESIGNER.md` §7.4, §10
- Требование ТЗ/task: decide which operation types/step_codes are enriched with `marketplace_listing_id`
- Затронутая область: operation row links, UI operation card visibility, backfill commands, reports, Stage 1/2 regression
- Почему нельзя продолжать без решения: choosing operation families changes user-facing operation detail links and backfill scope; this is a UX/functionality and regression-sensitive decision
- Рекомендация проектировщика: enrich only product/listing rows with deterministic unique same-store/same-marketplace listing match; exclude action/promotion summary rows
- Решение: enrichment applies to new operations and old operation rows where deterministic safe match exists. `OperationDetailRow.product_ref` remains immutable. Release evidence must include row count plus checksum/hash over `(id, product_ref)` before/after migration, enrichment and validation for existing rows.
- Remaining implementation constraints: no summary rows, no cross-store/cross-marketplace lookup, chunked/idempotent backfill, safe conflict logging and Stage 1/2 regression. Product code is not implemented by this decision.
- Дата решения: 2026-05-02
- Требуется эскалация заказчику через оркестратора: закрыта
- ADR: ADR-0044
...
- Статус: resolved/customer_decision
- Phase gate: closed_for_core_2_snapshot_filling_scope
- Blocking gate: нет for approved CORE-2 snapshot scope; future metrics still require separate specification.
- Где обнаружен: `docs/tasks/design/product-core/TZ_CORE_2_PRODUCT_CORE_INTEGRATION_FOR_CODEX_DESIGNER.md` §7.5, §10
- Требование ТЗ/task: decide which snapshot types are actually filled in CORE-2 and which remain contract/foundation
- Затронутая область: PriceSnapshot, StockSnapshot, SalesPeriodSnapshot, PromotionSnapshot, latest cache, UI/export semantics
- Почему нельзя продолжать без решения: sales, buyouts, returns and promotion semantics differ by marketplace and can drive future operational decisions if filled incorrectly
- Рекомендация проектировщика до решения: fill WB price snapshots, Ozon Elastic scoped stock snapshots, WB regular/Ozon Elastic promotion snapshots; keep future metrics foundation-only
- Решение: CORE-2 fills prices, stocks and promotions/actions if already available/approved. Sales, buyouts, returns, demand, in-work, production and shipments are future architecture hooks only: nullable foundation, not active CORE-2 UI/workflow, no formulas.
- Remaining implementation constraints: every filled snapshot needs approved source semantics, run/listing context, redaction and tests. Future analytical/operational formulas require separate design.
- Дата решения: 2026-05-02
- Требуется эскалация заказчику через оркестратора: закрыта
- ADR: ADR-0046
...
- Статус: resolved/customer_decision
- Phase gate: closed_for_core_2_external_normalization_mapping_workflow
- Blocking gate: нет, closed by customer decision 2026-05-02
- Где обнаружен: `docs/tasks/design/product-core/TZ_CORE_2_PRODUCT_CORE_INTEGRATION_FOR_CODEX_DESIGNER.md` §7.2, §10
- Требование ТЗ/task: decide whether CORE-2 needs import of mapping file from external article normalization program
- Затронутая область: Product Core import UI, file contour, mapping history, audit, object access, exports
- Почему нельзя продолжать без решения: importing external mapping is a user-facing workflow and can create/update Product Core relations; it cannot be designed as a hidden side effect
- Варианты решения:
  - A: no import; promo_v2 receives normalized articles only through WB/Ozon API
  - B: read-only import mapping report
  - C: full mapping import workflow with audit
- Рекомендация проектировщика: A for CORE-2 v1, or B if customer needs reconciliation visibility; C should be separate audited stage
- Решение: CORE-2 must support three matching modes:
  - normalized seller article already present via API -> auto-match under fixed internal SKU rules;
  - mapping table from external tool -> preview/diff/conflicts and explicit user confirmation before applying links;
  - manual mapping fallback.
- Invalid/non-unified external articles create/update `MarketplaceListing` only. UI actions: rerun/process via `visual_external`, upload/apply mapping table, or manual mapping.
- Remaining implementation constraints: mapping table apply must enforce object access, permissions, audit/history, redaction and conflict handling. External normalization program implementation itself remains out of CORE-2.
- Scope update for TASK-PC2-003 rework 2026-05-02: API auto-match remains in the narrowed TASK-PC2-003 slice; mapping table / `visual_external` workflow is deferred to a separate future task and is non-blocking for TASK-PC2-003.
- Дата решения: 2026-05-02
- Требуется эскалация заказчику через оркестратора: закрыта
- ADR: ADR-0043, ADR-0045

### GAP-CORE2-006: Auto-created InternalProduct shell policy for TASK-PC2-003

- Статус: resolved/customer_decision
- Phase gate: closed_for_narrowed_TASK_PC2_003_API_linkage_auto_create_slice
- Blocking gate: нет, закрыт решением заказчика 2026-05-02 для суженного TASK-PC2-003 API exact valid article linkage + auto-create/imported_draft.
- Где обнаружен: TASK-PC2-003 design handoff review 2026-05-02.
- Требование ТЗ/task: auto-create `InternalProduct` + imported/draft `ProductVariant` when a valid API article has no variant/conflict.
- Затронутая область: normalized article linkage, imported/draft variant lifecycle, visible internal product identity, audit/history, UI review/export labels.
- Проблема: `GAP-CORE2-001` approves Option B at policy level, and `CORE_2_MODEL_AND_MIGRATION_PLAN.md` requires creating/selecting a parent `InternalProduct` shell, but the task-scope docs do not define the required parent shell field policy. Implementation would otherwise have to guess `InternalProduct.internal_code`, `name`, `product_type`, `status`, category, comments, created_by/updated_by handling, and what to do when an `InternalProduct` with the candidate code already exists but has no matching variant.
- Почему нельзя продолжать без решения: auto-created `InternalProduct` records are visible business identity objects, not technical cache rows; choosing their code/name/type or parent selection behavior is business logic and UI-visible behavior.
- Решение:
  - `internal_sku` is the unique product/variant key for the current logic: one article = one unique variant.
  - At the first valid API article/internal_sku with no existing variant/product conflict, create the pair `InternalProduct` + `ProductVariant`.
  - `InternalProduct.internal_code = internal_sku`.
  - `InternalProduct.name` is taken from the marketplace title from the first load where the valid article is found; if title is blank, fallback to `internal_sku`.
  - `InternalProduct.product_type = finished_good`.
  - `InternalProduct.status = active`.
  - `InternalProduct.category = null`.
  - Human-readable traits/categories computed from the article are stored in `InternalProduct.attributes`; automatic category tree creation is prohibited.
  - `InternalProduct.comments = ""`.
  - `ProductVariant.internal_sku = internal_sku`; the variant name follows the same first marketplace title/fallback rule and the variant remains `review_state=imported_draft` until review.
  - A repeated same `internal_sku` from another store/marketplace must not create a new product; the listing links to the existing `ProductVariant`.
  - If the same `internal_sku` later arrives with a different human-readable title, do not overwrite the first `InternalProduct`/`ProductVariant` names. Store the new title on `MarketplaceListing.title`, keep the link `matched`, and set `ProductVariant.review_state = needs_review` for operator review. Do not infer title meaning.
  - If an existing single non-archived `InternalProduct.internal_code = internal_sku` exists but no variant exists and no other conflict exists, create the imported/draft `ProductVariant` under that product; do not create a second `InternalProduct`.
  - If the database contains an impossible `internal_sku` conflict or cannot safely select a unique active/non-archived product/variant, do not create a new product. Set the listing/mapping to `conflict` where the link cannot be selected safely; use `ProductVariant.review_state = needs_review` only for the safe title-mismatch review case after a unique variant was selected.
- Created/updated user attribution is not a separate business identity rule for this gap: implementation must follow the existing sync/audit convention (`sync_run.requested_by` when available, otherwise nullable/system context) and preserve safe audit/source context.
- Требуется эскалация заказчику через оркестратора: нет, решение получено 2026-05-02.
- Дата решения: 2026-05-02.
- ADR: ADR-0045.

### GAP-CORE2-007: External mapping table row contract for TASK-PC2-003

- Статус: deferred/future_task
- Phase gate: deferred_out_of_narrowed_TASK_PC2_003_to_future_external_mapping_table_workflow
- Blocking gate: нет for narrowed TASK-PC2-003 API exact valid article linkage + auto-create/imported_draft. Да only for any future external mapping table / `visual_external` table workflow.
- Где обнаружен: TASK-PC2-003 design handoff review 2026-05-02.
- Требование ТЗ/task: implement mapping table preview/diff/conflict/apply flow with explicit confirmation for external normalization table.
- Затронутая область: Product Core mapping import UI/service, file contour, object access, permissions, audit/history, redaction, tests.
- Проблема: `GAP-CORE2-005` approves the workflow and required preview categories, but task-scope docs do not define the table input contract: file format, required columns, source listing identity columns, store/account disambiguation, target `internal_sku` column name, allowed optional columns, row identity/idempotency, and whether the table upload is represented as an `Operation`, `MarketplaceSyncRun`, `FileVersion`, or UI-only preview object before apply.
- Почему нельзя продолжать без решения: file/schema and source-row identity determine user-facing preview results, access-denied rows, conflict detection and applied links. Choosing those rules in implementation would be a file-contour and UX/functionality guess.
- Решение по scope 2026-05-02:
  - External mapping table / `visual_external` table workflow is deferred to a separate future task.
  - TASK-PC2-003 now implements only API exact valid article linkage + auto-create/imported_draft.
  - TASK-PC2-003 must not implement mapping table preview/apply service, upload UI, apply UI, file schema, table persistence object, or `visual_external` workflow.
  - Deferred workflow remains non-blocking for the narrowed TASK-PC2-003 implementation slice.
- Future-task questions retained for the deferred mapping-table workflow:
  ...
  - What persistence object should preview/apply use in the future task: `Operation`, `MarketplaceSyncRun`, `FileVersion`, UI-only preview object, or another approved object?
  - If a valid target `internal_sku` has no `ProductVariant`, should explicit apply in the future mapping-table task create the same imported/draft product/variant as API auto-create, or should it remain a preview conflict until another UI task?
- Требуется эскалация заказчику через оркестратора: нет for narrowed TASK-PC2-003; да before implementing the deferred mapping-table/`visual_external` workflow.
- Дата решения: 2026-05-02.
- ADR: ADR-0043, ADR-0045.
