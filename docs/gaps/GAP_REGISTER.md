...
На 2026-05-02 CORE-2 documentation package documents mandatory open decisions from `docs/tasks/design/product-core/TZ_CORE_2_PRODUCT_CORE_INTEGRATION_FOR_CODEX_DESIGNER.md`. These gaps do not block documentation audit if clearly represented, but they block affected implementation slices until a decision is recorded here and, where needed, in `docs/adr/ADR_LOG.md`.

### GAP-CORE2-001: ProductVariant auto-create mode

- Статус: open/customer_decision_required
- Phase gate: blocks_before_core_2_auto_create_imported_draft_implementation
- Blocking gate: да, only for auto-create/imported-draft `ProductVariant` behavior in TASK-PC2-003 and related UI/model/export slices. Exact-match candidate marking/linkage may proceed after audit if the implementation task explicitly excludes auto-create and imported/draft lifecycle behavior.
...
- Требуется эскалация заказчику через оркестратора: да, requires customer decision via orchestrator
- ADR: ADR-0045 proposed/pending_customer_decision

### GAP-CORE2-002: Approved WB/Ozon listing sources

- Статус: open/source_and_customer_decision_required
- Phase gate: blocks_before_core_2_listing_sync_implementation_for_unapproved_sources
- Blocking gate: да, for any full listing sync endpoint not already approved in current docs/code
...
- Пробел: no approved full Ozon catalog/listings endpoint in current task package; Ozon sources are action-scoped, not full catalog
- Рекомендация проектировщика: implement WB price/listing adapter and Ozon Elastic scoped adapter only; block full Ozon listing sync until source is approved
- Требуется эскалация заказчику через оркестратора: да, for any business requirement to have full Ozon listing sync or new endpoint
- ADR: ADR-0042, ADR-0046

### GAP-CORE2-003: OperationDetailRow enrichment scope

- Статус: open/customer_or_orchestrator_decision_required
- Phase gate: blocks_before_core_2_operation_fk_backfill_and_new_row_enrichment_scope
- Blocking gate: да, for TASK-PC2-004 scope beyond a separately approved narrow slice
...
- Требуется эскалация заказчику через оркестратора: да, requires customer decision via orchestrator for final operation family scope
- ADR: ADR-0044

### GAP-CORE2-004: Snapshot scope

- Статус: open/customer_or_orchestrator_decision_required
- Phase gate: blocks_before_core_2_snapshot_filling_beyond_safe_minimum
- Blocking gate: да, for any snapshot type/source not approved in `CORE_2_SNAPSHOT_FILLING_SPEC.md`
...
- Рекомендация проектировщика: fill WB price snapshots, Ozon Elastic scoped stock snapshots, WB regular/Ozon Elastic promotion snapshots; keep SalesPeriodSnapshot foundation-only
- Требуется эскалация заказчику через оркестратора: да, for any broader business/analytics expectation; technical auditor may accept safe-minimum implementation if task scope excludes blocked types
- ADR: ADR-0046

### GAP-CORE2-005: External normalization mapping import

- Статус: open/customer_decision_required
- Phase gate: blocks_before_core_2_external_normalization_mapping_import
- Blocking gate: да, for any mapping file/report/import workflow from external normalization program
