# TASK-012-wb-api-prices-download.md

ID: TASK-012
Тип задачи: реализация Stage 2.1 / 2.1.1
...
- §6.6: 2.1.1 creates operation/files/product history/audit/techlog and must classify API operation explicitly.
- §10.1: secrets not stored outside protected secret reference.
- §11: operation/audit/techlog invariants.
- §16: no WB write in 2.1.1, no Stage 1 Excel logic change, no Ozon 2.2 mixing.

Разрешённые файлы / области изменения:
- Future path `apps/discounts/wb_api/prices/` or `apps/discounts/wb_api/price_*.py` - Prices API client/use case/normalizers.
...
- `apps/marketplace_products/` - WB product create/update/history from prices API.
- `apps/files/`, `apps/operations/`, `apps/audit/`, `apps/techlog/` - file/version, operation, audit and techlog integration for 2.1.1.
- `apps/exports/` - price Excel export writer if export code belongs there.
...
Ожидаемый результат:
- 2.1.1 operation has `mode=api`, `marketplace=wb`, `step_code=wb_api_prices_download`.
- `Operation.type` for this API step is `NULL` / blank / `not_applicable`, not `check/process`.
- `GET /api/v2/list/goods/filter` pagination continues to empty page.
- Price Excel with required columns is created and linked as output.
- Product directory update with history is created.
- Size conflict rows are visible and not upload-ready.

Критерии завершённости:
- 2.1.1 acceptance checklist passed.
- No WB write endpoints called.
- Tokens absent from metadata, snapshots, audit, techlog, UI, files, reports and test output.
- File versions and operation links are immutable.

Обязательные проверки:
- pagination/rate mock tests;
- size conflict tests;
- Excel schema tests;
