...
Матрица связывает обязательные требования этапа 1 с исполнительными документами, gaps и ADR. Статус `covered_with_artifact_gate` означает, что проектное правило отражено и gap закрыт, но формальная приёмка ждёт фактический customer artifact.
...
| §24 | Acceptance by control files | `docs/stages/stage-1/ACCEPTANCE_TESTS.md`, `docs/testing/TEST_PROTOCOL.md` | covered_with_artifact_gate | ADR-0013; customer files/checksums/expected results pending |
...
| blocks_before_acceptance/production | нет открытых GAP | production readiness policy утверждена; formal acceptance ждёт customer artifact gate по контрольным файлам |
